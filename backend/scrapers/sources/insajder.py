"""Insajder.net scraper — RSS-only (100 entries, Cloudflare+JS on article pages)."""
...
RSS_URL = "https://insajder.net/feed/"


class InsajderScraper(BaseScraper):
    SOURCE_ID = "insajder"

    # insajder.net has Cloudflare + JS on article pages.
    # RSS provides 100 entries with full content — use exclusively.
    ...
    def parse_article(self, url: str) -> Optional[ArticleData]:
        ...
        # Category: first tag is usually section
        ...
        # Fallback: try image from RSS content soup
        if not image_url:
            img_el = soup_rss.find("img")
            if img_el:
                image_url = img_el.get("src") or img_el.get("data-src")

        # Fetch full article HTML for complete text (insajder.net has RSS excerpt only)
        try:
